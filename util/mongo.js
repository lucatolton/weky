	...
	*/
	...
	async checkForBreaking(id, message) {
		await rpgSchema.findOne({ id: id }, async (err, data) => {
			...
			if (data.BerylP <= 0 && data.powerups.includes('Sword Agility')) return this.modify(id, 'powerups', 'Blood Eater II', 'delete')
			if (data.JadeiteP <= 0 && data.powerups.includes('Blood Eater II')) return this.modify(id, 'powerups', 'Sword Agility', 'delete')
