	...
	*/
	...
	async checkForBreaking(id, message) {
		await rpgSchema.findOne({ id: id }, async (err, data) => {
			...
			if (data.BerylP <= 0 && data.powerups.includes('Blood Eater II')) return this.modify(id, 'powerups', 'Perfect Shoot', 'delete')
			if (data.JadeiteP <= 0 && data.powerups.includes('Perfect Shoot')) return this.modify(id, 'powerups', 'Blood Eater II', 'delete')
