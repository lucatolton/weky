	...
	*/
	...
	async addUser(id, message) {
		...
		if (!data) {
			...
			if(message) message.channel.send('Heyo ' + message.author.tag + ', destiny here. I\'ve choosed you to be my hero. I will give you the identity of Dante. Goodluck in battles!');
		}
	},
	...
	async checkIfLvlUp(id, message) {
		...
		const byArgs = location.hero.find((val) => val.name.includes('Aspeon'));
		...
		const b = utils.randomizeNumber(1, 3);
		const i = utils.randomizeNumber(1, 5);
		if (db.heroMn >= db.heroLvlUpReq) {
			db.heroLvl += 1;
			const e = 500 * (1.5 + db.heroLvl);
			db.heroMn = db.heroMn / db.heroLvlUpReq;
			db.heroLvlUpReq += db.heroLvl;
			db.heroLvlUpReq *= 1.5;
			db.heroHp += 1;
			db.heroDmg += 1;
			this.addAero(id, e, message);
			await rpgSchema.findOneAndUpdate({ id: id }, data);
	...
	async addXP(id, message, xp) {
		await rpgSchema.findOne({ id: id }, async (err, data) => {
			...
			const multi = require('./utils').calculatePercentage(xp, 10);
	...
	async addAero(id, value, message) {
		await rpgSchema.findOne({ id: id }, async (err, data) => {
