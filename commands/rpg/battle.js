...
module.exports.run = async (client, message, args, utils, data) => {
	...
	await rpgSchema.findOne({ id: message.author.id }).lean().exec().then(async (extractedData) => {
		...
		if (extractedData.powerups.includes('Loot Domination I')) {
			...
			data.rpg.modify(message.author.id, 'TanzaniteP', utils.randomizeNumber(50, 10), '-=', message);
		}
		...
		ctx.fillStyle = 'red';
		ctx.fillText(gameData[1].health, 104, 25);
		ctx.fillText(gameData[0].health, 350, 25);
		ctx.font = '20px "Comic Sans"';
		ctx.fillStyle = '#0091FF';
		ctx.fillText(gameData[0].damage, 440, 25);
		ctx.fillText(gameData[1].damage, 29, 25);
		ctx.font = '30px "Comic Sans"';
		ctx.fillStyle = 'green';
		...
		const imageLink = new Discord.MessageAttachment(canvas.toBuffer(), 'pog.png');
		const embedBattle = new Discord.MessageEmbed()
		...
		message.reply({ content: str, embeds: [embedBattle], files: [imageLink] }).then(async (edity) => {
			...
			if (!checkHealth(player)) {
				while (bruh === true) {
					...
					const randNumb = Math.floor(Math.random() * gameData[Math.abs(tempPlayer - 1)].damage);

					if (chanceFor <= 5) {
						if (extractedData.powerups.includes('Blood Eater I')) {
							const percent = utils.calculatePercentage(gameData[tempPlayer].health, 5);
							gameData[tempPlayer].health -= Math.round(gameData[tempPlayer].health / percent);
							data.rpg.modify(message.author.id, 'BenitoiteP', utils.randomizeNumber(50, 10), '-=', message);
						}
						if (extractedData.powerups.includes('Healing Rod I')) {
							gameData[tempPlayer].health += 3;
							data.rpg.modify(message.author.id, 'GrandidieriteP', utils.randomizeNumber(50, 10), '-=', message);
						}
					}
					if (extractedData.powerups.includes('Blood Eater II') && chanceFor2 <= 10) {
						const percent = utils.calculatePercentage(gameData[tempPlayer].health, 5);
						gameData[tempPlayer].health -= Math.round(gameData[tempPlayer].health / percent);
						data.rpg.modify(message.author.id, 'BerylP', utils.randomizeNumber(50, 10), '-=', message);
						...
						if (extractedData.powerups.includes('Perfect Shoot') && gameData[tempPlayer].member === equipedHero.heroName) {
							...
							data.rpg.modify(message.author.id, 'PoudretteiteP', utils.randomizeNumber(30, 10), '-=', message);
						}
					}
					const a = utils.calculatePercentage(randNumb, 7);
					...
					if (checkHealth(player)) {
						...
						const imageL = new Discord.MessageAttachment(canva.toBuffer(), 'eagames.png');

						const embedBatt = new Discord.MessageEmbed()
							.setFooter(message.author.tag + '\'s battle')
							.addField(gameData[1].member, '<:TY:845324588887703552><:PE:845324642993307718> `' + Hero.typeOf + '`\n<:DM:845319973915459604><:DM:845320037875056712> `' + gameData[1].damage + '`\n<:HP:845326275035529287><:HP:845326308417601566> `' + gameData[1].health + '`', true)
							.addField('_ _', '**' + extractedData.stats.planet + '**', true)
							.addField(gameData[0].member, '<:LO:845326936636915716><:OT:845326999403757619> `' + Mob.chanceForThings + '%`\n<:DM:845319973915459604><:DM:845320037875056712> `' + gameData[0].damage + '`\n<:HP:845326275035529287><:HP:845326308417601566> ' + `\`${gameData[0].health}\``, true)
						...
						message.reply({ content: str, embeds: [embedBatt], files: [imageL] });
					}
				}
			}
			else {
