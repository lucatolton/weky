...
module.exports.run = async (client, message, args, utils, data) => {
	...
	var member = await data.rpg.user(message.author.id, message)
	if (member.db.aero < 50) return utils.errorEmbed(message, 'Battle costs 50 aero.')

	data.rpg.modify(message.author.id, 'aero', 50, '-=', message)

	if (member.db.powerups.includes('Loot Domination I')) {
	...
	const Hero = items.hero.find((val) => val.name.includes(member.equipedHero.heroName));
	...
	let gameData = [
		...
		{
			member: member.equipedHero.heroName,
			health: member.equipedHero.heroHp,
			damage: member.equipedHero.heroDmg,
		},
	];
	...
	let embedBattle = new Discord.MessageEmbed()
		...
		.addField('_ _', '**' + member.db.stats.planet + '**', true)
		...
		if (!checkHealth(player)) {
			async function fight() {
				while (bruh === true) {
					...
					if (gameData[tempPlayer].member === member.equipedHero.heroName) {
						if (member.db.powerups.includes('Damage Invocation')) {
					...
					if (checkHealth(player)) {
						...
						async function b() {
							...
							if (gameData[tempPlayer].member === Mob.name) {
								...
								if (!member.db.stats.mobsDefeated.includes(Mob.name)) data.rpg.modifyStats(message.author.id, 'mobsDefeated', Mob.name, 'push', message)
