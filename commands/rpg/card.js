...
module.exports.run = async (client, message) => {
	await rpgSchema.findOne({ id: message.author.id }).lean().exec().then(async (extractedData) => {
		...
		const total = equipedHero.heroMn;
		const needMore = Math.round(equipedHero.heroLvlUpReq);
		...
		const y = 300;
		const z = 288;
		const heroes = require('../../data/rpg-data');

		const daFinder = heroes.hero.find((val) => val.name.includes(equipedHero.heroName));
