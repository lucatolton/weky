...
module.exports.run = async (client, message, args, utils, data) => {
	...
	users.findOne({ id: target }, async (err, data) => {
		data.premium = true;
		data.save();
		message.channel.send(new Discord.MessageEmbed().setTitle('Success!').setDescription(`Premium set for **${client.users.cache.get(target).tag}** `).setColor('GREEN'));
		client.channels.cache.get('835918793166422016').send(new Discord.MessageEmbed().setTitle(`Premium added to ${client.users.cache.get(target).username}`).setColor('GREEN'));
	});
};
module.exports.help = {
