...
module.exports.run = async (client, message, args, utils, data) => {
	await rpgSchema.findOne({ id: message.author.id }).lean().exec().then(async (extractedData) => {
		await ssSchema.findOne({ SpaceShipID: extractedData.stats.inWhatSpaceShip }, async (err, ssData) => {
			...
			message.reply(utils.emojis.share + ` | **${message.author.username}** deposited \`${amount.toLocaleString("en") + ' (' + Math.round(percentageBetweenVault) + '%)` ' + utils.emojis.aero} to **${ssData.SpaceShipName + '#' + ssData.SpaceShipID}**.`)
		})
	})
};
module.exports.help = {
